...
1. This would be the set of rules in a .aggl file:
...
2. The initial world model is stored in a xml file:

    <AGMModel>
        <symbol id="1" type="A" />  #There is only a vertex node (symbol) with the identifier "1" and te type "A"
    </AGMModel>
    
3. The goal or target world is stored in another xml file:
...
1. In the top we have the AGM planner. Given an initial world and an objective world, it is in charge of generating a plan to reach the goal.
2. 
