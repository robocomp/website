...
### 8. Revision Design and Learnbot Functions

Learnbot is a low-cost educational robot designed to develop computational thinking through the python language.
In this revision we intend to rethink the design to make Learnbot a modular robot, so that the robot is built in phases increasing the difficulty in each of these phases.

These modules will depend on each other and can not be programmed and assembled before they have finished those on which they depend.
The incorporation of new modules will give learnbot new functions. It is about visualizing the level of learning through the construction of the robot giving an extra motivation to those who want to start in the world of programming and robotics.

Needed skills: Python, 3D modeling software  
Mentor: Juan Mario Haut  
Backup mentor: Pablo Bustos


### 9. Learnbot examples to use and simulator

Learnbot is a low-cost educational robot designed to develop computational thinking through the python language.
In this revision we intend to rethink the design to make Learnbot a modular robot, so that the robot is built in phases increasing the difficulty in each of these phases.

The first thing we will do is give the robot a simulator in which to work without needing to have the robot physically in front. We will create new examples that can be used in the school and in the institutes by the students.
...
In this work, the students updates all components, refactor the code and create tests for this framework, in such a way as to achieve an improvement in its stability.
...
### 11. Robust Robocomp code

Currently RoboComp allows robot to autonomously navigate in its surrounding. Different RoboComp components and agents concurrently run in order to make possible this navigation: localization, slam or path planner algorithms are examples of them. Besides, all these components use AGM graph models for cognitive world modeling.  

However,  considering the recent use of mobile robots in a more social context, the use of classical navigation techniques is restricted, since they do not differentiate humans from other ordinary objects in the environment. 

The task at hand here would be to integrate social behaviors in the RoboComp navigation Agent. A typical case of use would be different group of humans walking and interacting with the environment. All these simulations would be made using RCIS (RoboComp InnerModel Simulator) 
